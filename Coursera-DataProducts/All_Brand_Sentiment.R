...
n = 500
raw <- searchTwitter(searchterm, n=limit, lang='en', encoding='utf-8')
df <- twListToDF(raw)
...
Dataset$text_nosym <- str_replace_all(Dataset$text, "[^[:alnum:]]", '')
scores <- score.sentiment(Dataset, ratingDict, .progress='text') #Call Function
...
ggplot(stat, aes(time, score)) + geom_point(size=1) +
  ...
  ggtitle(paste(searchterm, as.Date(stat$created)))
